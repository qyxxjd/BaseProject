...
import com.classic.android.utils.ShellUtil.CommandResult;
 ...
 * <li>{@link PackageUtil#isTopActivity(Context, String)} whether the app whost package's name is
 * packageName is on the
 * top of the stack</li>
 * </ul>
 *
 * @author <a href="http://www.trinea.cn" target="_blank">Trinea</a> 2013-5-15
 */
@SuppressWarnings({"unused", "WeakerAccess"}) public final class PackageUtil {
     ...
     * 检查是否安装过某个应用
     */
    @SuppressWarnings("WrongConstant") public static boolean checkApkExist(
     ...
     * </ul>
     */
    public static int install(@NonNull Context context, @NonNull String filePath) {
     ...
     * <li>You should add android.permission.INSTALL_PACKAGES in manifest, so no need to request
     * root
     ...
     *
     * @param filePath file path of package
     * @return {@link PackageUtil#INSTALL_SUCCEEDED} means install success, other means failed.
     * details see
     * {@link PackageUtil}.INSTALL_FAILED_*. same to {@link PackageManager}.INSTALL_*
     * @see #installSilent(Context, String, String)
     */
    public static int installSilent(@NonNull Context context, @NonNull String filePath) {
    ...
    public static int installSilent(
            @NonNull Context context, @NonNull String filePath, @NonNull String pmParams) {
        File file = new File(filePath);
     ...
     */
    public static int uninstall(@NonNull Context context, @NonNull String packageName) {
        if (PackageUtil.isSystemApplication(context) || ShellUtil.checkRootPermission()) {
     ...
     */
    public static int uninstallSilent(@NonNull Context context, @NonNull String packageName) {
     ...
     * <ul>
     * <li>Don't call this on the ui thread, it may costs some times.</li>
     * <li>You should add android.permission.DELETE_PACKAGES in manifest, so no need to request
     * root
     * permission, if you are system app.</li>
     * </ul>
     *
     ...
     * </ul>
     */
    public static int uninstallSilent(
            @NonNull Context context, @NonNull String packageName, boolean isKeepData) {
        /**
         * if context is system app, don't need root permission, but should add <uses-permission
